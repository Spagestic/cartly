...
export type ChainOfThoughtStepProps = ComponentProps<"div"> & {
  ...
  /** Shown inline after the label when the step is collapsed (e.g. source favicons). */
  collapsedPreview?: ReactNode
  /** Collapse step body (default: true when there are children). */
  collapsible?: boolean
  open?: boolean
  defaultOpen?: boolean
  onOpenChange?: (open: boolean) => void
...
function hasStepBody(children: ReactNode): boolean {
...
export const ChainOfThoughtStep = memo(
  ({
    ...
    const hasBody = hasStepBody(children)
    const collapsible = collapsibleProp ?? hasBody

    const [isOpen, setIsOpen] = useControllableState({
      defaultProp: defaultOpen ?? status === "active",
      prop: openProp,
      onChange: onOpenChange,
    })
    ...
    const stepOpen = status === "active" ? true : isOpen
    ...
    return (
      ...
      >
        ...
        <div className="min-w-0 flex-1 space-y-2 overflow-hidden">
          {collapsible && hasBody ? (
            <Collapsible open={stepOpen} onOpenChange={setIsOpen}>
              <CollapsibleTrigger
                className={cn(
              ...
              >
                <span className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2 gap-y-1">
                  ...
                  {!stepOpen && collapsedPreview ? collapsedPreview : null}
                </span>
                <ChevronDownIcon
                  className={cn(
                    "size-3.5 shrink-0 text-muted-foreground transition-transform",
                    stepOpen ? "rotate-180" : "rotate-0"
                  )}
                />
              </CollapsibleTrigger>
              {description ? (
